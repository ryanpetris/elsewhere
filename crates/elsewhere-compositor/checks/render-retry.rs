...
fn main() {
    ...
    let memory = render_node.is_none() || std::env::var_os("ELSEWHERE_MEMORY_FRAMES").is_some();
    ...
    let compositor = elsewhere_compositor::spawn(elsewhere_compositor::Config {
        ...
        exec: None, exec_env: vec![], kiosk: true, frame_transport: if memory { elsewhere_core::FrameTransport::Memory } else { elsewhere_core::FrameTransport::Dmabuf },
    ...
    for mode in [1, 2, 3] {
        ...
        wait(|| probes[0].color.load(SeqCst) == color && (!memory || probes[2].color.load(SeqCst) == color));
        ...
        wait(|| probes[1].color.load(SeqCst) == color && probes[3].accepted.load(SeqCst) > 0
            && (!memory || probes[3].color.load(SeqCst) == color));
