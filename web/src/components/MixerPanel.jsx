  ...
  return (
    <article role="group" aria-label={`${node.name} ${directions[node.kind]}`} data-audio-id={node.id} className="card flex flex-col gap-2.5 p-3">
      ...
      <div className="flex items-center gap-2">
        ...
        </label> : <span className="flex-1 text-[11px] text-ink-4">Volume control unavailable</span>}
        ...
        </button> : <span className="text-[11px] text-ink-4">Mute control unavailable</span>}
      ...
      {controls && ((!node.volume_writable && node.volume !== null) || (!node.mute_writable && node.mute !== null)) && <p className="text-[11px] text-ink-4">Disabled controls are read-only for this audio object.</p>}
      {targetKind && <div className="flex flex-wrap items-center gap-2 text-[11px] text-ink-3">
        ...
        {targets.length > 1 && (!routing || !node.routing_writable) && <span className="text-ink-4">Routing control unavailable</span>}
      </div>}
      <div className="flex items-center gap-2 text-[11px] text-ink-3">
        <span className="w-12 shrink-0">Level</span>
        <meter min="0" max="1" value={node.meter_active ? Math.min(1, peak) : 0} aria-label={`${node.name} peak level`} aria-valuetext={level} className="meter flex-1" />
        <span className="w-16 shrink-0 text-right font-mono text-ink-2 tabular-nums">{level}</span>
      </div>
      <p className="-mt-1 text-[10px] text-ink-4">{node.meter_before_volume ? 'Metered before stream volume and mute' : 'Metered after volume and mute'}</p>
...
export function MixerPanel({ viewer, hidden, onClose }) {
  ...
  return (
    <section aria-label="Session audio mixer" hidden={hidden} className="flex max-h-[45vh] shrink-0 flex-col border-t border-line bg-surface text-sm">
      <div className="flex h-9 shrink-0 items-center gap-2 border-b border-line px-3">
        <SlidersHorizontal className="size-3.5 text-ink-3" />
        <h2 className="text-xs font-medium text-ink">Session audio mixer</h2>
        <span className="hidden text-[11px] text-ink-4 sm:inline">· changes affect every viewer; each application row is one audio stream</span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        {role !== 'controller' && <p className="callout callout-warn mb-3 text-xs">Only the controlling viewer can make changes.</p>}
        ...
        {status !== 'connected' ? <p className="text-xs text-ink-3">Waiting for the desktop connection.</p>
          : !snapshot.available ? <p className="text-xs text-ink-3">{snapshot.error || 'Connecting to session audio…'}</p>
