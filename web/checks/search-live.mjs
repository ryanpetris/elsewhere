...
const root = await mkdtemp('/tmp/elsewhere-search-live-');
await mkdir(root + '/runtime', { mode: 0o700 });
await mkdir(root + '/data/applications', { recursive: true });
await writeFile(root + '/data/applications/search.desktop', '[Desktop Entry]\nType=Application\nName=Search fixture\nComment=Open a Wayland window\nExec=foot --title=SearchLaunched sleep 600\n');
...
try {
  ...
  const openSearch = async query => { await page.keyboard.press('Control+Alt+Shift+S'); await input.waitFor(); await input.fill(query); };
  await openSearch('Duplicate');
  assert.equal(await page.getByRole('listbox', { name: 'Results' }).getByRole('option').count(), 2);
  ...
  await openSearch('Search fixture');
  await page.locator('[data-entry="app:search"]').click();
  await wait('launch', async () => (await windows()).some(w => w.title === 'SearchLaunched'));
  ...
  assert(!(await inputLog()).includes('sym: u'), 'search text cannot enter Wayland client');
  await openSearch('Terminal'); await page.locator('[data-entry="action:terminal"]').click();
  await input.waitFor({ state: 'hidden' });
  await page.waitForFunction(() => document.activeElement?.classList.contains('xterm-helper-textarea'));
  await page.getByRole('region', { name: 'Terminal' }).getByText('Connected', { exact: true }).waitFor();
  await page.keyboard.type('export SEARCH_MARK=retained'); await page.keyboard.press('Enter');
  await openSearch('Terminal'); await page.locator('[data-entry="action:terminal"]').click();
  await input.waitFor({ state: 'hidden' });
  await page.waitForFunction(() => document.activeElement?.classList.contains('xterm-helper-textarea'));
  await page.keyboard.type('printf "%s" "$SEARCH_MARK" > terminal-marker'); await page.keyboard.press('Enter');
  await wait('same terminal', async () => { try { return (await readFile(root + '/terminal-marker', 'utf8')) === 'retained'; } catch { return false; } });
  await openSearch('Fullscreen'); await input.press('Enter');
  await page.waitForFunction(() => elsewhere.isFullscreen());
  await page.keyboard.press('Control+Alt+Shift+H');
  await openSearch('Files'); await input.press('Enter');
  await page.waitForFunction(() => elsewhere.isFullscreen() && document.activeElement?.getAttribute('aria-label') === 'Files');
  const limited = await (await api('/api/tokens', 'POST', { label: 'Search fixture', permissions: ['desktop.view', 'apps.launch'] })).json();
  const reader = await browser.newPage();
  await reader.goto(origin + '/#token=' + limited.token);
  await reader.waitForFunction(() => elsewhere.store.get().status === 'connected');
  await reader.locator('#search-toggle').click();
  const search = reader.getByRole('combobox', { name: 'Search', exact: true });
  await search.fill('Search fixture');
  await reader.locator('[data-entry="app:search"]').waitFor();
