...
try {
  ...
  for (const kind of (process.env.SEMANTIC_TOOLKITS || 'gtk,qt,chromium,firefox').split(',')) {
    ...
    if (kind === 'gtk') {
      ...
      const missing = { target: { role: 'button', name: 'Never present' }, condition: 'present', timeout_ms: 10000 };
      const holds = Array.from({ length: 15 }, () => api(path + '/wait', 'POST', missing, reader.token));
      ...
      assert.equal((await api(path)).status, 429);
      controller.abort(); await disconnected;
      await wait('HTTP disconnect frees operation', async () => (await api(path)).status === 200);
      ...
      const held = Array.from({ length: 15 }, () => api(path + '/wait', 'POST', missing, heldKey.token));
      ...
      await wait('MCP cancellation frees operation', async () => (await api(path)).status === 200);
      await cancelled;
      await api('/api/tokens/' + heldKey.metadata.id, 'DELETE');
      assert((await Promise.all(held)).every(r => r.status === 401));
      console.log('HTTP disconnect, MCP read/cancellation and 16-operation bound passed');
      ...
      console.log('unresponsive accessibility deadline, pre-dispatch revocation and native snapshot passed');
      ...
      console.log('30 bounded waits', reads, 'tree reads;', calls, 'D-Bus calls;', Date.now() - started, 'ms;', (await cpu(server.pid) - initialCpu) / ticks, 'server CPU sec;', (await cpu(bus.pid) - initialBusCpu) / ticks, 'bus CPU sec; resource samples', samples);
