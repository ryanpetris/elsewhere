...
static struct workspace workspaces[4];
...
static void ws_removed(void *data, struct ext_workspace_handle_v1 *ws) { (void)data; (void)ws; abort(); }
...
static void group_caps(void *data, struct ext_workspace_group_handle_v1 *group, uint32_t caps) { (void)data; (void)group; assert(caps == 0); }
...
static void new_group(void *data, struct ext_workspace_manager_v1 *m, struct ext_workspace_group_handle_v1 *group) { (void)data; (void)m; ext_workspace_group_handle_v1_add_listener(group, &group_listener, NULL); }
static void new_workspace(void *data, struct ext_workspace_manager_v1 *m, struct ext_workspace_handle_v1 *ws) { (void)data; (void)m; assert(count < 4); workspaces[count].handle = ws; ext_workspace_handle_v1_add_listener(ws, &ws_listener, &workspaces[count]); count++; }
...
int main(int argc, char **argv) {
    ...
    for (unsigned i = 0; i < count; i++) { assert(workspaces[i].caps == EXT_WORKSPACE_HANDLE_V1_WORKSPACE_CAPABILITIES_ACTIVATE); if (workspaces[i].number == target) target_ws = &workspaces[i]; }
    assert(target_ws);
