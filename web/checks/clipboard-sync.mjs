...
try {
  ...
  const connect = async (token = admin, windowId = null) => {
    const context = await browser.newContext(), requests = [], controls = { holdWrite: false, release: null };
    await context.route('**/api/clipboard{,/state}', async route => {
      ...
      await delay(100);
      if (request.method() === 'PUT' && controls.holdWrite) {
        controls.holdWrite = false;
        controls.operation = (await response.json()).operation;
        await new Promise(resolve => { controls.release = resolve; });
      }
      if (request.method() === 'GET' && record.path === '/api/clipboard' && controls.holdRead) {
  ...
  if (!baseline) {
    ...
    await windowViewer.page.context().close();
    for (const dispose of [false, true]) {
  ...
  await browser?.close(); server.kill('SIGTERM'); await log.close(); await rm(root, { recursive: true, force: true });
}
