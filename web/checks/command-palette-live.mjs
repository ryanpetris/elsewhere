...
try {
  ...
  const openPalette = async query => { await page.locator('#apps-toggle').click(); await input.waitFor(); await input.fill(query); };
  ...
  const search = reader.getByRole('combobox', { name: 'Search' });
