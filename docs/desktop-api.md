...
encoder sink the server made for it (`SinkFactory` in `elsewhere-server`, a `FfmpegSink` per stream).
On content changes, redraw requests or a stream's retry deadline, `render_window_streams` renders
the relevant windows' elements (popups too, within the
...
window) are skipped. Missing buffers and transient refusal keep that stream pending for the next
tick; a frame-cap refusal schedules a complete picture at its deadline. Initialization and encoder
pressure wait for the worker's redraw notification. Per-stream timed and next-tick retries leave
unchanged healthy streams idle.
Streams of windows no longer on the desktop are dropped, which drops their sinks and
